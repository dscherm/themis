...
### Tamper history — detection feeds the next prediction

Routing has one input besides the human-set policy: evidence from previous runs. When the green phase catches a broken seal — a sealed test whose fingerprint changed, or a forged red-state record — the gate appends the event to `.themis/blind_tdd/tamper_ledger.jsonl` along with the task's operator-authored metadata (id, declared paths, tags). On every later run, before anything is spawned, the gate matches the current task against that ledger; a task **similar** to a previously tampered one (a re-run of the same task, a task in the same directory, or a task sharing a tag) is escalated into the gate even if the selective policy would have skipped it.

The reasoning is worth stating because it constrains what the ledger is allowed to do. Detection is inherently after the fact — a hash break means the tampering already ran, and nothing can contain that run retroactively. So the break is used only as a *learning signal*: it upgrades the prediction for the next run of anything similar, instead of pretending to protect the run it came from. The escalation match is also surfaced in the gate result (`details.tamper_escalation`) and callable directly (`blind_tdd.escalation.evaluate_escalation(task)`), which is the hook a stricter containment tier — running the whole gated task inside an OS-level sandbox — would use to make that call **before** any agent spawns.

Two properties keep the ledger honest:
